...
The printout groups items into one block per day. Columns are *Journal, Entry, Partner, Reference, Label, Debit, Credit, Difference* (plus *Currency* for multi-currency users). Each day closes with a **Day total** row, and the report closes with a **Total** row.
...
The printout has one block per account, ordered by account code, with columns *Date, Journal, Entry, Partner, Reference, Label, Debit, Credit, Balance*. Each block closes with an **Account total** row; the report closes with a **Total** row.
