...
- **Check-out limit** — an employee cannot hold more than **3** checked-out
  ...
  together, so the batch stops as soon as the limit would be exceeded. The
  limit applies only to the **Check Out** button — it is not enforced on
  direct record imports or API writes that set the status to *Checked Out*.
